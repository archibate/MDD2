 ...
 * @version 0.17.6.10   2023/06/21
 *          - 为组播行情增加身份认证处理, 支持通过查询服务完成组播身份认证处理 (API接口以及现有配置和行为均保持兼容)
 *              - 通过将组播地址配置为查询服务地址, 来实现对组播行情的身份认证处理
 *              - API接口以及现有配置和行为均保持兼容
 *                - 对于 MdsApi_InitUdpChannel2 接口, 可以通过 remoteCfg.targetCompId 来指定组播频道类型
 *                  (如果是通过 MdsApi_ParseConfigFromFile 等接口解析的配置信息, 则会自动填充 targetCompId 字段, 无需额外设置)
 *                - 新增 MdsApi_InitUdpChannel3 接口, 以方便指定组播频道类型
 *
 * @since   2016/03/02
 */
 ...
 *
 * @deprecated  已过时, 建议使用 MdsApi_InitUdpChannel3 接口, 以方便指定组播频道类型.
 *              作为变通方案, 也可以将 remoteCfg.targetCompId 字段设置为如下取值, 来指定组播频道类型:
 ...
 * @param[out]  pUdpChannel     UDP行情订阅通道的会话信息
 * @param       pRemoteCfg      远程主机配置信息
 * @param       udpChannelType  组播频道类型
 *                              - MDS_UDP_CHANNEL_TYPE_SNAP1
 *                              - MDS_UDP_CHANNEL_TYPE_SNAP2
 *                              - MDS_UDP_CHANNEL_TYPE_TICK1
 *                              - MDS_UDP_CHANNEL_TYPE_TICK2
 * @retval      TRUE            成功
 * @retval      FALSE           失败。此时 errno 将被设置, 可以通过 errno/SPK_GET_ERRNO() 获取到具体失败原因
 *
 * @exception   EINVAL          配置异常或传入参数非法
 ...
 * @exception   ETIMEDOUT       连接超时
 */
BOOL    MdsApi_InitUdpChannel3(
                MdsApiSessionInfoT *pUdpChannel,
                const MdsApiRemoteCfgT *pRemoteCfg,
                eMdsUdpChannelTypeT udpChannelType);
 ...
 *
 * @return  通过 MdsApi_SetThreadPassword 方法设置的登录密码的加密类型
