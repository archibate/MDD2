 ...
 */
        ...
        || defined (__clang__)
#   undef   __no_inline
#   define  __no_inline                 __spk_no_inline
#   define  __spk_no_inline             __attribute__ ((__noinline__))

#elif defined (__WINDOWS__) && defined (_MSC_VER)
#   ifndef  __no_inline
#       define  __no_inline             __spk_no_inline
#   endif
#   define  __spk_no_inline             __declspec(noinline)

#else
#   ifndef  __no_inline
#       define  __no_inline
#   endif
#   define  __spk_no_inline
 ...
 */
        ...
        || (defined (__INTEL_COMPILER) && __INTEL_COMPILER >= 800) \
        || defined (__clang__)
#   define  __spk_likely(expr)          __builtin_expect (!!(expr), 1)
#   define  __spk_unlikely(expr)        __builtin_expect (!!(expr), 0)
#   define  __spk_expect(expr, value)   __builtin_expect ((expr), (value))

#else
#   define  __spk_likely(expr)          (expr)
#   define  __spk_unlikely(expr)        (expr)
#   define  __spk_expect(expr, value)   (expr)
...
#   define  likely(expr)                __spk_likely(expr)
#endif
...
#   define  unlikely(expr)              __spk_unlikely(expr)
#endif
/* -------------------------           */
...
/* 返回按64字节对齐的SIZE */
#undef  SPK_ALIGN_SIZE64
#define SPK_ALIGN_SIZE64(SIZE)          SPK_ALIGN_SIZE((SIZE), 64)

/* 返回按128字节对齐的SIZE */
#undef  SPK_ALIGN_SIZE128
#define SPK_ALIGN_SIZE128(SIZE)         SPK_ALIGN_SIZE((SIZE), 128)
...
#elif defined (__WINDOWS__) && defined (_MSC_VER)
#   define  __spk_bswap32(x)            _byteswap_ulong(x)
#   define  __spk_bswap64(x)            _byteswap_uint64(x)

#else
    static __spk_always_inline uint32 __spk_bswap32(uint32 x) {
        return __SPK_SWAP_INT32(x);
    }

    static __spk_always_inline uint64 __spk_bswap64(uint64 x) {
        return __SPK_SWAP_INT64(x);
    }
...
#elif defined (__WINDOWS__) && defined (_MSC_VER)
#   define  __spk_bswap16(x)            _byteswap_ushort(x)

#else
    static __spk_always_inline uint16 __spk_bswap16(uint16 x) {
        return __SPK_SWAP_INT16(x);
    }
...
#define __spk_bswap16_direct(x)                                         \
        ( x = __spk_bswap16(x) )


/* Swap 4 byte, 32 bit values direct */
#define __spk_bswap32_direct(x)                                         \
        ( x = __spk_bswap32(x) )


/* Swap 8 byte, 64 bit values direct */
#define __spk_bswap64_direct(x)                                         \
        ( x = __spk_bswap64(x) )
/* -------------------------           */
...
#   define  SPK_SLEEP_NS(NS)                                            \
    do { \
        long            __SLEEP_NS_nsec = (NS); \
        struct timespec __SLEEP_NS_ts = { __SLEEP_NS_nsec >= 1000000000 ? \
                __SLEEP_NS_nsec / 1000000000 : 0, \
                __SLEEP_NS_nsec % 1000000000 }; \
...
#   define  SPK_SLEEP_NS(NS)                            \
    do { \
        long            __SLEEP_NS_nsec = (NS); \
        Sleep(__SLEEP_NS_nsec > 1000000 ? __SLEEP_NS_nsec / 1000000 \
                : __SLEEP_NS_nsec > 1000 ? 1 : 0); \
    } while (0)
...
#   define  SPK_SLEEP_NS(NS)                                            \
    do { \
        long            __SLEEP_NS_nsec = (NS); \
        struct timeval  __SLEEP_NS_tv = { __SLEEP_NS_nsec >= 1000000000 ? \
                __SLEEP_NS_nsec / 1000000000 : 0, \
                (__SLEEP_NS_nsec % 1000000000) / 1000 }; \
        select(0, (fd_set *) NULL, (fd_set *) NULL, (fd_set *) NULL, \
                &__SLEEP_NS_tv); \
    } while (0)
...
# elif !defined (__WINDOWS__) || defined (__GNUC__)
#   define SPK_ACCESS_ONCE(x)           (* ((volatile typeof(x) *) &(x)) )

# elif defined (__WINDOWS__) && defined (_MSC_VER) && _MSC_VER >= 1600
#   define SPK_ACCESS_ONCE(x)           (* ((volatile decltype(x) *) &(x)) )

# else
#   define SPK_ACCESS_ONCE(x)           (x)
#   pragma  message("Warning: Not support ACCESS_ONCE!")
