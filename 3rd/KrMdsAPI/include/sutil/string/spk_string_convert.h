...
/*
 * int64整型数值转为十六进制形式的字符串
 */
char*   SStr_Ll16toStr(char *pBuf, int64 num);

/*
 * int64整型数值转为三十二进制形式的字符串
 */
char*   SStr_Ll32toStr(char *pBuf, int64 num);
 ...
 * 转换带单位的数值型字符串->double
 */
int32   SStr_ParseUnittedDouble(const char *pStr, double *pOutValue);

/*
 * 转换带单位的数值型字符串->double
 */
int32   SStr_ParseUnittedDouble2(const char *pStr, double *pOutValue,
                double defaultValue);
 ...
 */
static __inline int64
SStr_StrtoLL16(const char *pValue) {
    return __spk_strtoll(pValue, (char **) NULL, 16);
}


/**
 * 解析16进制数值型字符串 (若参数为空则返回0)
 *
 * @param   pValue  待转换的数值字符串
 * @return  转换后的数值
 */
static __inline int64
SStr_StrtoLL16Nullable(const char *pValue) {
    if (__spk_unlikely(! pValue)) {
        return 0;
    }
    return SStr_StrtoLL16(pValue);
}
 ...
 */
static __inline int64
SStr_FixedStrtoLL16(const char *pValue, int32 len) {
    char            __tmpBuf[32];
    ...
    return SStr_StrtoLL16(__tmpBuf);
}
 ...
 */
static __inline int64
SStr_StrtoLL32(const char *pValue) {
    return (int64) __spk_strtoll(pValue, (char **) NULL, 32);
}


/**
 * 解析32进制数值型字符串 (若参数为空则返回0)
 *
 * @param   pValue  待转换的数值字符串
 * @return  转换后的数值
 */
static __inline int64
SStr_StrtoLL32Nullable(const char *pValue) {
    if (__spk_unlikely(! pValue)) {
        return 0;
    }
    return SStr_StrtoLL32(pValue);
}
 ...
 */
static __inline int64
SStr_FixedStrtoLL32(const char *pValue, int32 len) {
    char            __tmpBuf[32];
    ...
    return SStr_StrtoLL32(__tmpBuf);
}
/* -------------------------           */
