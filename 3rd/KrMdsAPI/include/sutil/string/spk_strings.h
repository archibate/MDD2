...
/*
 * 按指定分隔符分割传入字符串, 并对每个区段执行回调函数(不会修改传入的字符串)
 */
int32   SStr_ForeachSection(const char *pStr, const char *pDelimChars,
            int32 (*fnCallback)(const char*, void*, void*, void*),
...
#if defined(__MINGW__) && defined(__GNUC__) && (__GNUC__ >= 7)
#   pragma GCC diagnostic push
...
SStr_GetParentSection(const char *pStr, const char *pDelimChars, char *pBuf,
    ...
    return SStr_StrCopy(pBuf, pStr, (int32) SPK_MIN(bufSize - 1, pBase - pStr));
}
 ...
 * 按字节比较内存数据是否均为指定的取值
 *
 * @param   pMem                待比较的内存数据地址
 * @param   value               待匹配的数值
 * @param   size                待比较的内存数据长度
 *                              - @note 长度小于等于0时, 直接返回 FALSE
 * @return  TRUE 所有字节均为指定的取值; FALSE 不全为指定的取值
 */
static __inline BOOL
SStr_MemChCmp(const void *pMem, unsigned char value, int32 size) {
    unsigned char       *pSrc = (unsigned char *) pMem;
