...
typedef struct _SEndpointChannelCfg {
    ...
    /**
     * 连接断开后的回调函数
     * - 无需做特殊处理, 通信端点会自动尝试重建连接
     * - 若函数指针为空, 通信端点会自动尝试重建连接并继续执行
     * - 若回调函数返回小于0的数, 则通信端点将中止运行
     *
 ...
 *
 * <p> 线程说明:
 * - OnMsg/OnConnect/OnDisconnect 回调函数均运行在通信线程或连接管理线程下
 * </p>
 *
 ...
 *
 * <p> 回调函数说明:
 * - 仅用于通知客户端连接已经断开, 无需做特殊处理, 通信端点会自动尝试重建连接
 * - 若函数指针为空, 通信端点会自动尝试重建连接并继续执行
 * - 若回调函数返回小于0的数, 则通信端点将中止运行
 * </p>
 *
