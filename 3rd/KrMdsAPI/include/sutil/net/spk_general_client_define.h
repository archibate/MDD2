...
/** 最大的心跳间隔(秒) */
#define GENERAL_CLI_MAX_HEARTBEAT_INTERVAL              (7200)
...
typedef struct _SGeneralClientChannel {
    ...
    int32               testReqInt;         /**< 测试请求/上行链路心跳间隔, 单位为秒 */
    ...
    /** 保留给服务器或API内部使用的, 用于存储自定义数据的扩展缓存区 */
    union {
        struct {
            /** 保留给服务器或API内部使用的缓存区 */
            SDataBufferT
        ...
        /** 缓存行填充域 */
        char            __padding[SPK_CACHE_LINE_SIZE];
    } __reserveBuf;
...
typedef struct _SGeneralClientRemoteCfg {
    ...
    int16               uplinkHeartBtInt;   /**< 上行链路心跳间隔, 单位为秒 (解析配置时, 若未配置则默认设置为 -1) */
    int16               downlinkHeartBtInt; /**< 下行链路心跳间隔, 单位为秒 (解析配置时, 若未配置则默认设置为 -1) */
