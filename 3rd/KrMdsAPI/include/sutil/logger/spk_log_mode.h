 ...
 */
/** 日志内容的最大长度 (16K) */
#define     SLOG_MAX_INFO_SIZE      (16384)
/* XXX 日志内容的最大长度
...
typedef struct _SLogMode {
    uint8               mode;                       /**< 日志模式代码 */
    uint8               appenderType;               /**< 日志记录器类型 @see SLOG_TYPE_FILE */
    uint8               isAsyncLog;                 /**< 是否是异步日志 */
    uint8               isFileDaily;                /**< 是否每天一个日志文件  */
...
/** 日志模式 - 控制台日志 (CONSOLE, 等同于 CONSOLE_STDOUT) */
#define SLOG_MODE_CONSOLE               (&__SPK_slogModes[4])

/** 日志模式 - 控制台日志 (CONSOLE_STDOUT, 输出到标准输出 stdout) */
#define SLOG_MODE_CONSOLE_STDOUT        (&__SPK_slogModes[5])

/** 日志模式 - 控制台日志 (CONSOLE_STDERR, 输出到标准错误 stderr) */
#define SLOG_MODE_CONSOLE_STDERR        (&__SPK_slogModes[6])

/** 日志模式 - 异步文件日志 (ASYNC_FILE, 轮换, 不区分具体日期) */
#define SLOG_MODE_ASYNC_FILE            (&__SPK_slogModes[7])

/** 日志模式 - 异步文件日志 (ASYNC_FILE_DAILY, 每天一个日志文件) */
#define SLOG_MODE_ASYNC_FILE_DAILY      (&__SPK_slogModes[8])
/* -------------------------           */
