...
bool udp_quote_base::init(sock_udp_param& param) {
	m_udp_param = param;
	...
	* get them. */
	...
	/// 开始接收线程
	m_recv_thread.reset(new std::thread(&udp_quote_base::work_thread, this));
	return true;
}
...
void udp_quote_base::work_thread() {
	bind_thread(*m_recv_thread, m_udp_param.m_cpu_id);
